export default function SearchBox(widget) {
    const defaultSearchUrl =
        widget.dataset.defaultSearchUrl;
    const target = widget.dataset.target || "_blank";
    const newTab = widget.dataset.newTab === "true";
    const suggestionsEnabled = widget.dataset.suggestionsEnabled === "true";
    const widgetId = widget.dataset.widgetId;
    const inputElement =
        widget.getElementsByClassName("search-input")[0];
    const bangElement =
        widget.getElementsByClassName("search-bang")[0];
    const dropdownElement =
        widget.getElementsByClassName("search-shortcuts-dropdown")[0];
    const shortcutsListElement =
        widget.getElementsByClassName("search-shortcuts-list")[0];
    const kbdElement =
        widget.getElementsByTagName("kbd")[0];
    ...
    // Initialize bangs
    const bangs =
        widget.querySelectorAll(".search-bangs > input");
    ...
    function hideErrorIndicator() {
        ...
        const errorIndicator =
            widgetContainer?.querySelector('.widget-header .notice-icon-major');
        if (errorIndicator) {
    ...
    function highlightItem(index) {
        const items =
            shortcutsListElement.querySelectorAll('.search-shortcut-item');
